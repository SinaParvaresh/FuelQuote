...
const Login = (props) => {
    ...
    async function checkUserInfo() {
        ...
        const response = await fetch('http://localhost:5000/login/authentication', {
            method: 'POST',
    ...
    return (
        <div className='login-background'>
            <form onSubmit={submitHandler} className="Login" action="/profileManagement">
                <h1>
                    LOGIN
                </h1>
                <br />
                ...
                <div className="input-group mb-3">
                    ...
                    <input onChange={event => setPassword(event.target.value)} value={password} type="password" className="form-control" aria-label="Username" aria-describedby="basic-addon1" required />
                </div>
                <div>
                    <button type='submit' className="btn btn-primary" onClick={checkUserInfo}>Submit</button>
                </div>
                <br />
